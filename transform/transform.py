...
def batch(iterable, n=1, extra=0):
    l = len(iterable)
    for ndx in range(extra, l, n):
        yield iterable[ndx - extra:min(ndx + n, l)]
...
@click.option('-v', '--vertical-trim', type=click.FLOAT, help='Vertical trim (top and bottom) as a percent', default=0.0)
@click.option('-h', '--horizontal-trim', type=click.FLOAT, help='Horizontal trim (top and bottom) as a percent', default=0.0)
@click.option('-t', "--out-dtype", type=cli.NUMPYTYPE, default=np.uint8, help="Datatype of Output.")
@click.option('-p', '--processes', type=click.INT, help='Process Count (for simulatenous images)', default=psutil.cpu_count())
...
def norm(normalize_over, data_dir, output_dir, vertical_trim, horizontal_trim, out_dtype, processes, mips):
	...
	with tf.TiffFile(inputs[0]) as tif:
		...
		out_dim = np.s_[int(dim[0] * horizontal_trim):int(dim[0] * (1 - horizontal_trim)), int(dim[1] * vertical_trim):int(dim[1] * (1 - vertical_trim))]
			...
			if mips > 1:
				with norm_mem as mips_mem:
					for i in reversed(indices[mips_offset:]):
						# log.log("Making MIP", f"{i}:{i - mips_offset}:{mips_mem.shape}:{np.min(mips_mem[i])}:{np.max(mips_mem[i])}", log_level=log.DEBUG.INFO)
