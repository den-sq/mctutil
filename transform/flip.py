...
def transpose_write(recon_mem: SharedNP, path, i):
	""" Writes a portion of reconstruction to disk after transposing it.
		...
		:param i: Slice(s) of reconstruction to write.
	"""
...
@click.command()
@click.option("-f", "--flip-axis", type=click.INT, required=True, help="Dimension (0/1/2/etc) to flip image stack in.")
...
def flip_stack(flip_axis, inputdir, outputdir):
