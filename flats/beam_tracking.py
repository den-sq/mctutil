...
def diagnose(pre, post, beam_sigma=8, dark_quantile=0.15):
    ...
    print("-- beam-change diagnosis (revised) ------------------")
    print(f"  bit depth        : {'8-bit round-trip' if bd['eight_bit'] else 'OK'} "
          f"({bd['levels']} levels, step {bd['step']:.3g}, max {bd['vmax']:.0f})")
    print(f"  intensity drift  : {info['intensity_drift_pct']:+.1f} %")
    print(f"  noise floor      : {sig:.3f} ADU  ({info['noise_rel_pct']:.2f}% of signal)")
    print(f"  smooth motion    : {info['logR_smooth_std']:.4f}  (logR)")
    print(f"  high-freq logR   : {info['logR_highfreq_std']:.4f}  "
          f"(lag-1 autocorr {lag1:.2f} -> "
          f"{'MOVED structure' if lag1 > 0.3 else 'noise-like'})")
    print(f"  est. fine motion : {fine_motion:.4f}   noise part {logR_noise:.4f}")
    print(f"  dark-region quant: {info['dark_rel_quant_pct']:.1f}% (frame-bottom rel. noise)")

    motion_dominated = info["motion_to_noise"] > 3 and (lag1 > 0.3 or fine_motion > 2*logR_noise)
    print("  -- recommendation --")
    if not motion_dominated:
        print("  noise comparable to motion -> optimal_transport_interp (method 2).")
    elif bd["eight_bit"] or dark_rel_q > 0.01:
        print("  motion-dominated, but 8-bit/dark degradation present:")
        print("    * method 2 is the safe choice on THIS data;")
        print("    * reprocess from 12-bit median'd flats, then use")
        print("      flow_warp_interp / separated_warp_interp (methods 3/4).")
    else:
        print("  clean & motion-dominated -> flow_warp_interp (3) or")
        print("  separated_warp_interp (4) to recover fine-band ghosting.")
    print("-----------------------------------------------------")
    return info
...
def estimate_flow_ratio_HS(pre, post, beam_sigma=4, env_sigma=40,
    ...
    if verbose:
        print(f"  flow: median v={np.median(v):+.2f} u={np.median(u):+.2f} px, "
              f"99th-pct |disp|={md:.2f} px")
    if max_disp is not None and md > max_disp:
        print(f"  ** WARNING: |disp| {md:.1f}px > max_disp {max_disp}px -- possible "
              f"band aliasing; clamping. Check beam periodicity / reduce motion.")
...
def save_decomposition(flats, out_dir, names=None, beam_sigma=4, **decomp_kw):
    ...
    paths = [os.path.join(out_dir, "static_defects.tif")]
    tifffile.imwrite(paths[0], D)
    for k, nm in enumerate(names):
        p = os.path.join(out_dir, f"dynamic_beam_{nm}.tif")
...
def calibrate_schedule(projs, scan_fractions, pre, post, mask=None,
    ...
    if verbose:
        print("beam-time calibration (from air regions):")
        for f, ti in zip(fr, tstar):
            print(f"  scan-frac {f:.3f} -> beam_t {ti:+.3f}")
        if kind == "linear":
            print(f"  linear fit: beam_t = {info['alpha']:+.3f} "
                  f"+ {info['beta']:.3f}*scan_frac  (resid {info['resid']:.3f})")
            print(f"  -> first projection already at beam_t {info['alpha']:.2f}; "
                  f"NOT 0. Pass schedule= into any interpolator.")
    return sched, info
...
if __name__ == "__main__":
    import sys, tifffile
    pre = tifffile.imread(sys.argv[1] if len(sys.argv) > 1 else "pregain.tif")
    post = tifffile.imread(sys.argv[2] if len(sys.argv) > 2 else "postgain.tif")
    diagnose(pre, post)
    print("\nbuilding methods (flow estimated on downsampled grid)...")
    methods = {
        "crossfade": geometric_crossfade(pre, post),
        "optimal":   optimal_transport_interp(pre, post),
        "flow_warp": flow_warp_interp(pre, post, max_disp=20),
        "separated": separated_warp_interp(pre, post, max_disp=20),
    }
    lq = np.log(_clean(post))
    print("\nendpoint check  std(log G(1) - log post):")
    for name, G in methods.items():
        print(f"  {name:10s}: {np.std(np.log(_clean(G(1.0))) - lq):.4f}")
    print("\nG(t) ready; use flatfield_projection(proj, G, t).")
